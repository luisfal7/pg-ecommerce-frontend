...
export function postUser(value) {
    return async function (dispatch) {
        const create = await axios.post(
            `https://pi-dogs-backend.onrender.com/users`,
            value
        );
        return dispatch({
        type: "POST_USER",
...
export const getUsers = () => (dispatch)=>{
    return axios('https://pi-dogs-backend.onrender.com/users')
                    .then(res => dispatch({type: 'GET_USERS', payload: res.data}))
}

export const getClients = () => (dispatch)=>{
    return axios('https://pi-dogs-backend.onrender.com/cart')
...
export const getReviews = (shoeId) => (dispatch)=>{
    return axios(`https://pi-dogs-backend.onrender.com/reviews/${shoeId}`)
        .then(res => dispatch({type: 'GET_REVIEWS', payload: res.data}))
}

export const getExactReview = (shoeId, idUser) => (dispatch) => {
    return axios(`https://pi-dogs-backend.onrender.com/reviews/${shoeId}`, {idUser : idUser})
        .then(res => dispatch({type: 'GET_EXACT_REVIEW', payload: res.data}))
}

export function postReview(idUser, review, rating, shoeId) {
    return async function (dispatch) {
    const create = axios.post(`https://pi-dogs-backend.onrender.com/reviews/${shoeId}`, { idUser: idUser, review: review, rating: rating })
        return dispatch({
        type: 'POST_REVIEW',
        payload: create,
        });
...
export const editReview = (idReview, review, rating) => (dispatch) =>{
    return axios.put(`https://pi-dogs-backend.onrender.com/reviews/exact/${idReview}`, {review, rating })
    .then(res => dispatch({type: 'EDIT_REVIEW', payload: res.data}))
}

export const deleteReview = (idReview) => (dispatch) => {
    return axios.delete(`https://pi-dogs-backend.onrender.com/reviews/exact/${idReview}`)
...
export const getFavorites = (idUser) => (dispatch)=>{
    return axios(`https://pi-dogs-backend.onrender.com/favorites/${idUser}`)
    .then(res => dispatch({type: 'GET_FAVORITES', payload: res.data}))
}

export function postFavorite(idUser, shoeId) {
    return async function (dispatch) {
    const create = axios.post(`https://pi-dogs-backend.onrender.com/favorites/${idUser}`, { shoeId: shoeId })
        return dispatch({
        type: 'POST_FAVORITES',
        payload: create,
        });
...
export const deleteFavorite = (idUser, shoeId) => (dispatch) => {
    return axios.delete(`https://pi-dogs-backend.onrender.com/favorites/${idUser}`, { data: {shoeId} })
...
export const getOrders = (email) => (dispatch) => {
 return axios(`https://pi-dogs-backend.onrender.com/cart/${email}`)
 .then(res => dispatch({type: 'GET_USER_ORDERS', payload: res.data}))
}

export const getOrderId = (idPayment) => (dispatch) => {
 return axios(`https://pi-dogs-backend.onrender.com/cart/order/${idPayment}`)
